# builder for the Rust crate & Python wheel
FROM python:3.12 AS builder

WORKDIR /workspace
COPY src/rust_image_gen/ .
...
WORKDIR /app
RUN mkdir wheels && mkdir src

COPY --from=builder --chown=app_user:app_user /workspace/target/wheels/ ./wheels
COPY pyproject.toml .
COPY /src ./src
